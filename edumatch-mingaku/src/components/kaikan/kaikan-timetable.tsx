...
const VENUES = [
  { key: "国際会議室", label: "国際会議室", bg: "bg-emerald-50", border: "border-emerald-200", ring: "ring-emerald-500", selectedBg: "bg-emerald-100", text: "text-emerald-800" },
  { key: "ステージ①", label: "ステージ①円卓", bg: "bg-blue-50", border: "border-blue-200", ring: "ring-blue-500", selectedBg: "bg-blue-100", text: "text-blue-800" },
  { key: "ステージ②", label: "ステージ②", bg: "bg-amber-50", border: "border-amber-200", ring: "ring-amber-500", selectedBg: "bg-amber-100", text: "text-amber-800" },
] as const;
...
function SessionBlock({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={[
        "absolute left-1 right-1 rounded-lg border p-1.5 text-left text-[11px] leading-tight transition overflow-hidden",
        ...
        !disabled && !selected ? "hover:ring-1 hover:ring-primary/40 cursor-pointer" : "",
      ].join(" ")}
      style={{ top, height }}
              ...
              {slots.map((slot) => {
                ...
                return (
                  <div key={slot} className="absolute right-0 left-0" style={{ top }}>
                    <span className={`absolute -top-2 right-1 text-[10px] tabular-nums ${isHour ? "font-bold text-foreground" : "text-muted-foreground"}`}>
                      {slot}
                    </span>
                  </div>
                );
              ...
              <div key={venue.key} className="relative border-l">
                ...
                {slots.map((slot) => {
                  ...
                  return (
                    <div
                      key={slot}
                      className={`absolute left-0 right-0 ${isHour ? "border-t border-border/60" : "border-t border-border/20 border-dashed"}`}
                      style={{ top }}
                    />
                  );
                })}
