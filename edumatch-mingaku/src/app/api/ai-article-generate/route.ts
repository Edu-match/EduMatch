...
## 本文（content）の構成（必ずこの順序・見出しで）

Markdown の ## 見出しを使い、**前半→後半**の流れを守ってください。

**前半：参照元の要約**
- ## 参照元の要約  
  元コンテンツの内容を事実ベースでわかりやすく整理する（背景・要点・注目点など）。

**後半：読者・教育者が生かせる部分**
- ## 教育現場でのアイデア  
  授業・校務・ICT 活用など、実践・応用の視点で具体的に書く。
- ## 💡先生へのポイント  
  箇条書き（- ）を活用。明日から役立つアクション・留意点を整理する。
- ## まとめ  
  2〜3文で、読後の行動や検討のきっかけを示す。

- サブセクションが必要な場合は ### を使ってよい。
- 全体で1000〜2500文字程度。
- **重要**：本文の末尾に「---」や「出典：」による参照・URLは**一切書かない**（システムがユーザーの入力URLで自動付与する）。
...
/** 画像例と同形式：--- のあと「出典：「…」」+ Markdown リンク */
function buildSourceAttributionBlock(sourceUrl: URL, quotedTitle: string): string {
  const href = sourceUrl.href;
  const label = quotedTitle.trim() || defaultSourceQuotedTitle(sourceUrl);
