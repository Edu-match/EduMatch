...
提供された元コンテンツのテキストを元に、教育関係者・EdTech企業向けの記事を生成してください。
...

{
  ...
  "tags": "タグ（カンマ区切り、3〜5個）",
  "thumbnailKind": "domestic | overseas | recruitment | other のいずれか（サムネイルテンプレート色。下記ルール）"
}

## title / leadText と content の分担

- **title**：記事のメイン見出し（例：企業・製品名＋ニュース性のある一行）。
- **leadText**：その下に続く短いリード（1〜2文。記事の要点・読者メリット）。
- **content**：ここには **title / leadText を書かない**。**本文だけ**書く。

## 本文（content）の書き方・完成イメージ

次のトーンに近づけること（見出し文言は**一例に過ぎない**。内容に合わせてAIが**すべての見出しを創作**すること）。

- **見出し（中見出し）**：エディタでは「見出し」ブロックとして扱う。**行頭に \`## \`（半角シャープ2つ＋半角スペース）** を付けた1行を見出しとする（サイト上では中見出し・太字系のスタイルになる）。\`#\`（大見出し）や \`###\`（小見出し）は**使わない**。見出しを \`**太字**\` の通常段落にしない（見出しと本文を混同しない）。
- **改行・空行**：見出し行の**直後に空行を入れない**（改行1回のあと、そのまま本文）。**連続する空行は入れない**。話題の区切りは \`---\` のみ。**\`---\` の前後にも空行を入れない**（\`---\` は単独行）。段落を分けるときだけ、必要最小限の1行空けにとどめる。
- **区切り**：大きな話題の切り替えごとに **単独行で** \`---\`（3つのハイフン）。
- **前半（複数セクション可）**：参照元の**要約・説明**を中心に書く。固定文言「参照元の要約」などの見出しは禁止。各ブロックは \`## 見出し\` ＋ 本文。
- **中盤〜後半**：仕組み・影響・展望・課題なども、同様に \`##\` 見出し＋本文。
- **💡 先生へのポイント**：上記のあと \`---\` を挟み、**次の行を一字不差で** \`## 💡 先生へのポイント\`（💡と「先生」のあいだは**半角スペース1つ**）とし、**改行1回のあと**本文（箇条書き可）。見出し直後に空行を入れない。
- **まとめ**：\`---\` を挟み、**次の行に** \`## まとめ\` とし、**改行1回のあと**2〜4文で総括。見出し直後に空行を入れない。

全体で1000〜2500文字程度。

**重要**：本文の末尾に「---」や「出典：」や URL は**一切書かない**（システムがユーザーの入力URLで自動付与する）。

## thumbnailKind（サムネイル用テンプレート）

記事の性質に最も近いものを1つ選ぶ：

- **domestic**：国内の教育ニュース・国内企業・国内の学校向けトピックが中心
- **overseas**：海外の動向・海外サービス・国際比較が中心
- **recruitment**：採用・募集・求人・応募・講師募集などが主題
- **other**：上記に当てはまらない、その他
...
- 誇大表現・根拠のない断言は避ける
- 元コンテンツを元にしつつ、EduMatch読者向けに編集・加工してよい`;
