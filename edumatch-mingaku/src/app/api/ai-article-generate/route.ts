...
次のトーン・体裁に近づけること（見出し文言は**一例に過ぎない**。内容に合わせてAIが**すべての段落タイトルを創作**すること）。

- **見出し**：Markdown の \`#\` / \`##\` / \`###\` は**使わない**。各セクションの先頭に、**1行だけ** \`**太字**\`（\`** ... **\`）で囲んだ日本語タイトルを置く（例：\`**「AIダッシュボード」とは何か？先生が知っておくべき基本**\` のような問いかけ・要約行でよい）。
- **区切り**：大きな話題の切り替えごとに、**単独行で** \`---\`（3つのハイフン）を入れる。
- **前半ブロック（複数セクション可）**：参照元の内容の**要約・説明**を中心に書く。事実ベースで、背景・発表内容・要点を整理する。**「参照元の要約」などの固定見出しは禁止**（中身が要約であることと、見出しテキストを同一にしない）。
- **中盤〜後半**：仕組み・具体例・教育現場への影響・展望・留意点など、読者が行動・判断に使える論点を、同じく**AIが考えた見出し＋段落**で展開する。
- **💡 先生へのポイント**：上記のあと \`---\` を挟み、**次の行に一字不差で** \`**💡 先生へのポイント**\`（💡と「先生」のあいだは**半角スペース1つ**、全体を太字タグで囲む）と書き、その下に段落（箇条書き可）で実践的な助言を書く。
- **まとめ**：\`---\` を挟み、**次の行に** \`**まとめ**\` とだけ書き（太字）、その下に2〜4文で総括する。
