...
import { checkPromptInjection, checkLlmOutput, rateLimitResponse, verifyOrigin } from "@/lib/security";
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // 内部エラーの詳細はクライアントに流さない（情報漏洩防止）
        const message = "AIの応答生成に失敗しました。しばらくしてからお試しください。";
        try {
